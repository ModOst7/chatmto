...
app.AppView = Backbone.View.extend({
	...
	initialize: function(name) {
		...
		app.Chat.set({name: this.name, sessionId: socket.socket.sessionid})
		this.listenTo(app.Chat, 'change', this.render);
		app.Chat.save();
		socket.on('messageToUser',  _.bind(function(data) {
			data.message.message = rc4(socket.socket.sessionid, data.message.message);
	...
	sendMessage: function() {
		...
		message.message = rc4(socket.socket.sessionid, message.message);
